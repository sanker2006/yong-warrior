...
const V3 = (() => {
  ...
  async function renderToday() {
    ...
    const primaryTitle = promotion && promotion.nextLevel
      ? `${promotion.currentLevel} → ${promotion.nextLevel} 晋级进度`
    ...
    container.innerHTML = `
      <section class="v31-today-command animate-in">
        ...
        <div class="v31-today-command__meta">
          <span class="badge badge--amber">${escapeHtml(state.user.level || user.level || 'L0')}</span>
          <span class="badge badge--ghost">${user.role === 'instructor' ? '管理员' : '学员'}</span>
          ${promotion?.nextLevel ? `<span class="badge badge--cyan">${escapeHtml(promotion.weekRange)} / ${escapeHtml(promotion.nextLevel)}</span>` : (nextLesson ? `<span class="badge badge--cyan">${escapeHtml(nextLesson.week.title)} / ${escapeHtml(nextLesson.lesson.code)}</span>` : '<span class="badge badge--cyan">COURSE CLEAR</span>')}
        </div>
        ${courseGlyph(nextLesson?.lesson, 'v31-today-command__glyph')}
      </section>

      ${promotion?.nextLevel ? `
      <section class="promotion-panel animate-in animate-in-delay-1">
        <div class="promotion-panel__head">
          <span>PROMOTION ROUTE</span>
          <strong>${escapeHtml(promotion.currentLevel)} → ${escapeHtml(promotion.nextLevel)}</strong>
        </div>
        <div class="promotion-track"><i style="width:${Math.max(0, Math.min(100, promotion.progressPercent || 0))}%"></i></div>
        <div class="promotion-grid">
          <div><span>阶段课程</span><strong>${promotion.course.completedLessons}/${promotion.course.requiredLessons}</strong></div>
          <div><span>课程题正确率</span><strong>${promotion.quiz.answered ? promotion.quiz.accuracy + '%' : '待答题'}</strong></div>
        </div>
        <p>${escapeHtml(promotion.blockers?.[0] || '当前等级已达到最高阶段。')}</p>
      </section>
      ` : ''}
    ...
    container.innerHTML = `
      <div class="v31-lesson-shell">
        ...
        <div class="lesson-tabs lesson-tabs--v31 animate-in animate-in-delay-1">
          ...
          <button class="lesson-tab ${lessonTab === 'quiz' ? 'active' : ''}" onclick="V3.switchLessonTab('quiz')">关联题目</button>
        </div>
    ...
    container.innerHTML = `
      <div class="quiz-header animate-in">
        ...
        <div class="quiz-filter-status">
          当前：${state.quizType === 'course' ? '课程题' : '情景题'} / ${escapeHtml(state.quizFilterLesson || state.quizFilterWeek || '全部课程')} / ${escapeHtml(state.quizFilterSkill || '全部知识域')}
        </div>
  ...
  async function renderProfile() {
    ...
    const weak = profile.weakRecommendations || [];
    const promotion = profile.promotion || null;
    ...
    container.innerHTML = `
      ...
      <section class="promotion-panel animate-in animate-in-delay-2">
        ...
        <p>${escapeHtml(promotion.blockers?.[0] || '完成对应课程并且阶段课程题正确率大于 90% 后自动晋级。')}</p>
        <div class="v31-action-row mt-3">
          ${promotion.nextActions?.[0]?.type === 'course' ? `<button class="btn btn--primary btn--sm" onclick="V3.openLesson('${promotion.nextActions[0].target.split('-')[0]}', '${promotion.nextActions[0].target}')">继续课程</button>` : ''}
      ...
      <section class="promotion-panel mt-4 animate-in animate-in-delay-2">
        <div class="promotion-panel__head">
          <span>自动晋级节点</span>
          <strong>${escapeHtml(promotion?.currentLevel || user.level || 'L0')}</strong>
        </div>
  ...
  return {
    ...
    setQuizType, setQuizWeek, setQuizSkill, clearQuizFilters,
