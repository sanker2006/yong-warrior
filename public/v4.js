...
const V4 = (() => {
  ...
  const TABS = ['today', 'course', 'quiz', 'activities', 'profile'];
  const SUB_VIEWS = ['course-detail', 'lesson', 'quiz-session', 'aar', 'assessment', 'activity-detail', 'my-activities'];
  ...
  function navigate(view, opts = {}) {
    ...
    const titles = {
      ...
      activities: '活动', 'activity-detail': opts.title || '活动详情',
      'my-activities': '我的活动',
  ...
  function goBack() {
    ...
    } else if (v === 'activity-detail' || v === 'my-activities') {
      navigate('activities');
    } else {
  ...
  function renderView(view, opts) {
    switch (view) {
      ...
      case 'activity-detail': renderActivityDetail(opts.activityId); break;
  ...
  async function renderProfile() {
    ...
    container.innerHTML = `
      ...
      <div class="flex flex-col gap-2 animate-in animate-in-delay-3">
        <button class="btn btn--secondary btn--block" onclick="V4.navigateMyActivities()">活动记录</button>
        <button class="btn btn--secondary btn--block" onclick="V4.navigateAar()">复盘记录</button>
        <button class="btn btn--danger btn--block" onclick="V4.logout()">退出登录</button>
      </div>
    `;
    ...
    container.querySelectorAll('.weak-rec__train').forEach(btn => {
      btn.addEventListener('click', (event) => {
        ...
        V4.startQuizWithSkill(btn.dataset.skill || '');
      });
    });
    ...
    try {
      ...
      container.innerHTML = `
        <div class="empty-state mt-6">
          ...
          <button class="btn btn--secondary btn--sm mt-3" onclick="V4.switchTab('activities')">重试</button>
        </div>
      `;
    }
  }
  ...
  async function renderActivityDetail(activityId) {
    ...
    try {
      ...
      if (!activity) {
        container.innerHTML = `
          <div class="empty-state mt-6">
            <div class="empty-state__text">活动不存在</div>
            <button class="btn btn--secondary btn--sm mt-3" onclick="V4.switchTab('activities')">返回活动列表</button>
          </div>
        `;
        return;
      }
      ...
      container.innerHTML = `
        <div class="empty-state mt-6">
          <div class="empty-state__text">加载失败</div>
          <button class="btn btn--secondary btn--sm mt-3" onclick="V4.goBack()">返回</button>
        </div>
      `;
    }
  }
  ...
  function openActivityDetail(activityId) {
    navigate('activity-detail', { activityId, title: '活动详情' });
  }

  // 报名活动
  async function registerForActivity(activityId) {
    const btn = $('#activity-action-btn');
    if (btn) {
      btn.disabled = true;
      btn.textContent = '提交中...';
    }
  ...
  async function cancelActivityRegistration(activityId) {
    ...
    try {
      await api(`/activities/${activityId}/cancel`, { method: 'POST' });
      toast('已取消报名', 'success');
      renderActivityDetail(activityId);
    } catch (err) {
  ...
  async function renderMyActivities() {
    ...
    `;
    try {
      const data = await api('/activities/my');
  ...
  return {
    ...
    closeModal,
    // Activities
    renderActivities, renderActivityDetail, openActivityDetail,
    registerForActivity, cancelActivityRegistration,
    renderMyActivities, navigateMyActivities
  };
