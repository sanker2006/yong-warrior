...
const V3 = (() => {
  ...
  function renderCourseDetail(weekId) {
    ...
    container.innerHTML = `
      <div class="course-detail__header animate-in">
        <div class="course-detail__phase">${phaseName}</div>
        <div class="course-detail__title">${week.title}</div>
        <div class="course-detail__week-badge">
          <span class="badge badge--cyan">${week.week}</span>
          ${relatedQs.length > 0 ? `<span class="badge badge--ghost">${relatedQs.length} 道题</span>` : ''}
        </div>
      </div>

      <div class="info-block animate-in animate-in-delay-1">
        <div class="info-block__title">// 目标</div>
        <div class="info-block__text">${weekContent ? weekContent.objective : week.objective}</div>
      </div>

      <div class="info-block animate-in animate-in-delay-1">
        <div class="info-block__title">// 标准</div>
        <div class="info-block__text">${weekContent ? weekContent.standard : week.standard}</div>
      </div>

      ${weekContent && weekContent.warning ? `
      <div class="info-block animate-in animate-in-delay-2" style="border-left-color:var(--accent-amber)">
        <div class="info-block__title" style="color:var(--accent-amber)">// 注意</div>
        <div class="info-block__text">${weekContent.warning}</div>
      </div>
      ` : ''}
      ...
      ${week.lessons.map((l, i) => {
        ...
        return `
          <div class="lesson-card ${done ? 'completed' : ''} animate-in animate-in-delay-${Math.min(i + 2, 4)}"
  ...
  function renderLessonDetail(lessonId, weekId) {
    ...
    container.innerHTML = `
      <div style="padding-top:var(--sp-3)">
        ...
        ${lesson.image ? `
        <div class="info-block animate-in animate-in-delay-1" style="text-align:center;padding:var(--sp-3)">
          <img src="${lesson.image.src}" alt="${lesson.image.alt || ''}" style="width:100%;max-height:200px;object-fit:contain;border-radius:var(--radius-md)">
        </div>
        ` : ''}

        <div class="lesson-tabs animate-in animate-in-delay-1">
  ...
  function renderQuizList() {
    ...
    if (state.quizFilterWeek) {
      filtered = filtered.filter(q => questionMatchesWeek(q, state.quizFilterWeek));
    }
  ...
  function getFilteredQuestions() {
    ...
    if (state.quizFilterWeek) {
      qs = qs.filter(q => questionMatchesWeek(q, state.quizFilterWeek));
    }
    if (state.quizFilterSkill) {
      qs = qs.filter(q =>
        q.skillClassification === state.quizFilterSkill ||
  ...
  async function renderProfile() {
    ...
    // Build radar data from real question attempts. Fall back to assessment dimensions if present.
    const abilityItems = Array.isArray(profile.ability) ? profile.ability : [];
    const dimensions = abilityItems.length
      ? Object.fromEntries(abilityItems.map(item => [item.domain, item.score || 0]))
      : (profile.dimensions || {});
    const dimLabels = abilityItems.length
      ? Object.fromEntries(abilityItems.map(item => [item.domain, item.domain]))
      : Object.fromEntries(Object.keys(dimensions).map(key => [key, key]));
