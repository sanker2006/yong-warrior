  ...
  if (state.bankScope === "course") {
    ...
    questions = questions.filter(question => {
      if (state.selectedLessonCode) {
        return questionMatchesLesson(question, state.selectedLessonCode);
      }
      return questionMatchesWeek(question, week.week);
    });
  }
...
function relatedQuestions(week) {
  return data.questionBank.filter(question => questionMatchesWeek(question, week.week));
}

function relatedQuestionsForLesson(lesson) {
  return data.questionBank.filter(question => questionMatchesLesson(question, lesson.code));
}

function courseCodes(question) {
  return Array.from(new Set([
    questionLessonMapping(question),
    ...(String(questionSource(question)).match(/W\d+-L[\dA-Z]+/g) || [])
  ].filter(Boolean)));
}

function questionMatchesLesson(question, lessonCode) {
  return courseCodes(question).includes(String(lessonCode || ""));
}

function questionMatchesWeek(question, weekCode) {
  const target = String(weekCode || "");
  return courseCodes(question).some(code => code.split("-")[0] === target);
}
