...
function createStore(options) {
  ...
  return {
    ...
    registerForActivity(userId, activityId, options = {}) {
      ...
      try {
        const row = db.prepare("SELECT * FROM activities WHERE id = ?").get(activityId);
        if (!row) return rollbackResult(db, { error: "活动不存在" });
        const activity = activityFromRow(row);
        if (activity.status !== "pending" && activity.status !== "active") return rollbackResult(db, { error: "活动已结束，不能报名" });
        ...
        if (row.max_participants > 0) {
          ...
          if (count >= row.max_participants) return rollbackResult(db, { error: "报名人数已满" });
        }
        if (wantsRental && row.max_rental_equipment > 0) {
          ...
          if (rentalCount >= row.max_rental_equipment) return rollbackResult(db, { error: "租赁设备名额已满" });
        }
        const existing = db.prepare("SELECT * FROM activity_registrations WHERE activity_id = ? AND user_id = ?").get(activityId, userId);
        let registration;
        if (existing) {
          if (existing.status === "registered") return rollbackResult(db, { error: "已报名" });
          ...
          registration = { id: existing.id, activityId, userId, status: "registered", rentEquipment: wantsRental };
        } else {
          ...
          registration = { id: regId, activityId, userId, status: "registered", rentEquipment: wantsRental };
        }
        db.exec("COMMIT");
    ...
    getUserActivityRegistration(userId, activityId) {
      const row = db.prepare("SELECT * FROM activity_registrations WHERE activity_id = ? AND user_id = ? AND status = 'registered'").get(activityId, userId);
      return row ? regFromRow(row) : null;
    },
...
function rollbackResult(db, result) {
  db.exec("ROLLBACK");
  return result;
}
