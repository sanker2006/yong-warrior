...
function createStore(options) {
  ...
  return {
    ...
    listActivities(status) {
      ...
      sql += " ORDER BY CASE status WHEN 'active' THEN 0 WHEN 'pending' THEN 1 WHEN 'ended' THEN 2 WHEN 'cancelled' THEN 3 ELSE 4 END, start_time DESC";
      return db.prepare(sql).all(...params).map(activityFromRow);
    },
    getActiveActivities() {
      const now = new Date().toISOString();
      return db.prepare(
        "SELECT * FROM activities WHERE status = 'active' OR (status = 'pending' AND (signup_deadline IS NULL OR signup_deadline >= ?)) ORDER BY start_time ASC"
      ).all(now).map(activityFromRow);
    },
    ...
    createActivity(user, body) {
      ...
      // 报名截止时间：如果未指定，默认为活动开始时间；如果活动已开始则不限制报名
      const signupDeadline = body.signupDeadline ? String(body.signupDeadline) : startTime;
      db.prepare(`
        INSERT INTO activities (id, title, description, location, start_time, end_time, signup_deadline, max_participants, status, created_by, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        id,
    ...
    updateActivity(activityId, body) {
      ...
      if (body.endTime !== undefined) { fields.push("end_time = ?"); params.push(String(body.endTime)); }
    ...
    registerForActivity(userId, activityId) {
      ...
      if (activity.status !== "pending") return { error: activity.status === "active" ? "活动已开始，不能新增报名" : "该活动当前不可报名" };
      const now = new Date().toISOString();
      if (activity.signup_deadline && now > activity.signup_deadline) return { error: "报名已截止" };
      if (activity.max_participants > 0) {
        ...
        if (count >= activity.max_participants) return { error: "报名人数已满" };
      }
      ...
      if (existing) {
        ...
        db.prepare("UPDATE activity_registrations SET status = 'registered', cancelled_at = NULL WHERE id = ?").run(existing.id);
        return { success: true, registration: { id: existing.id, activityId, userId, status: "registered" } };
      }
      const regId = crypto.randomUUID();
      db.prepare("INSERT INTO activity_registrations (id, activity_id, user_id, status, created_at) VALUES (?, ?, ?, 'registered', ?)")
        .run(regId, activityId, userId, now);
      return { success: true, registration: { id: regId, activityId, userId, status: "registered" } };
    },
