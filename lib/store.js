...
function createStore(options) {
  ...
  return {
    ...
    getActiveActivities() {
      ...
      return db.prepare(
        "SELECT * FROM activities WHERE status IN ('active', 'pending') AND (signup_deadline IS NULL OR signup_deadline >= ?) ORDER BY start_time ASC"
    ...
    registerForActivity(userId, activityId) {
      ...
      if (activity.status !== "active" && activity.status !== "pending") return { error: "该活动当前不可报名" };
      const now = new Date().toISOString();
